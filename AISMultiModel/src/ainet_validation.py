...
#                     matrix2[i][j - nr_of_attr_mat1] = matrix[i][j]
    ...
    def write(self, filename, lines_centers, lines_radius, nr_centers, radii):
        unit_cell_centers = np.reshape(nr_centers, (len(radii), 2))
        
        with open(filename, 'r') as c_file:
            # read a list of lines into data
            data = c_file.readlines()    
            # access the lines that contains the center parameters and set them
            for i in range(len(lines_centers)):     
                edt_line = ' ' * 21           
                edt_line += '(make cylinder (center ' + \
                            str(unit_cell_centers[i][0]) + ' ' + str(unit_cell_centers[i][1]) + ' 0.0) ' + \
                            '(radius ' + str(radii[i]) + ') (height infinity)\n'
                data[lines_centers[i]] = edt_line
                                        
        # and write everything back
        with open(filename, 'w') as c_file:
            c_file.writelines(data)
            
    def are_rods_into_cell(self, nr_centers, radii, x_min, x_max, y_min, y_max, dim):
        for i in range(len(radii)):
            current_x = nr_centers[i * dim]
            current_y = nr_centers[i * dim + 1]
    ...
    def is_inside_bounds(self, Ab, x_min, x_max, y_min, y_max, nr_of_centers, dim):
        ...
        # split nr_centers and radii data
        nr_centers = Ab[0:(nr_of_centers * dim)]
        radii = Ab[(nr_of_centers * dim):]
        # validate if rods are inside the unit cell
        if self.are_rods_into_cell(nr_centers, radii, x_min, x_max, y_min, y_max, dim):
            return True
        else:
            return False
